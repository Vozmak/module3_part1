...
import { Query } from './gallery.interface';
...
export const addImageGallery: Handler<APIGatewayLambdaEvent<null>, any> = async (event) => {
  ...
  try {
    ...
    //@ts-ignore
    const userUploadId: string = event.requestContext.authorizer.claims._id;

    return await manager.uploadImages(images, userUploadId);
  } catch (error) {
    return errorHandler(error);
  }
};
